...
void OutputHexData(const char* type, const std::vector<uint8_t>& data) {
    ...
    output_stream << std::uppercase << std::setfill('0') << std::setw(2) << std::right << std::hex;
    for (auto byte : data) {
        output_stream << byte;
    }
...
bool EMVGetPDOLAnswer(const std::vector<uint8_t>& aid, std::vector<uint8_t>& pdol_answer) {
    if (aid.size() > 255) {
        return false;
    }
    ...
    std::vector<uint8_t> apdu {
        0x00, 0xA4, 0x04, 0x00, static_cast<uint8_t>(aid.size())
    };
...
bool EMVGetPanFromData(const std::vector<uint8_t>& data, std::vector<uint8_t>& pan) {
...
bool EMVGetPDOL(const std::vector<uint8_t>& pdol_answer, std::vector<uint8_t>& pdol) {
    ...
    while (in_index < pdol_in.size()) {
        ...
        for (const auto& fixed_value : fixed_values) {
            if (fixed_value.tag == tag && fixed_value.data.size() == length) {
                ...
                pdol_out.insert(pdol_out.end(), fixed_value.data.begin(), fixed_value.data.end());
                break;
            }
        }
...
bool EMVGetDataByPDOL(const std::vector<uint8_t>& pdol, std::vector<uint8_t>& data) {
    std::vector<uint8_t> ber;

    // Select GPO
    // Header: 0x00 0xA4 0x04 0x00 len+2 0x83 len, Data: PDOL
    std::vector<uint8_t> apdu {
        0x80, 0xA8, 0x00, 0x00, static_cast<uint8_t>(pdol.size() + 2), 
        0x83, static_cast<uint8_t>(pdol.size())
    };
...
bool EMVGetAFLFromData(const std::vector<uint8_t>& data, std::vector<uint8_t>& afl) {
    Parser tlv_parser(data);
    if (tlv_parser.IsNull()) {
        return false;
    }
...
bool EMVReadRecord(uint8_t sfi, uint8_t record_id, std::vector<uint8_t>& record_data) {
    // Read record
    // Header: 0x00 0xA4 0x04 0x00 len+2 0x83 len, Data: PDOL
    uint8_t sfi_param = (sfi << 3) | (1 << 2);

    return nfc.ApduExchange({0x00, 0xB2, record_id, sfi_param, 0x00}, record_data, 1000);
}

bool EMVGetPanFromAFL(const std::vector<uint8_t>& afl, std::vector<uint8_t>& pan) {
