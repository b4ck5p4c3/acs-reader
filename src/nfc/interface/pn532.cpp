...
bool PN532::FindTag(NFCTagInfo& info, uint32_t timeout) {
    ...
    if (!this->CommandExchange({
        PN532_COMMAND_INLISTPASSIVETARGET_REQUEST,
        1,
        0
    }, answer, timeout)) {
        return false;
    ...
    info.uid.clear();
    info.uid.insert(info.uid.begin(), answer.begin() + 7, answer.begin() + 7 + uid_length);
    info.ats.clear();
    if (answer.size() == (uid_length + 7)) {
        return true;
    }
    ...
    info.ats.insert(info.ats.begin(), answer.begin() + 7 + uid_length, answer.begin() + 7 + uid_length + ats_length);
    return true;
}

bool PN532::ApduExchange(const std::vector<uint8_t>& in_data, std::vector<uint8_t>& out_data, uint32_t timeout) {
...
bool PN532::SendCommandData(const std::vector<uint8_t>& data) {
    ...
    if (data.size() <= 253) {
        ...
        command_data.push_back(~(static_cast<uint8_t>((command_length >> 8) + (command_length & 0xFF))) + 1);
    }

    command_data.push_back(PN532_HOSTTOPN532);
    command_data.insert(command_data.end(), data.begin(), data.end());

    uint8_t checksum = std::accumulate(data.begin(), data.end(), 0) + PN532_HOSTTOPN532;

    command_data.push_back(~checksum + 1);
    command_data.push_back(PN532_POSTAMBLE);

    this->wire_.beginTransmission(PN532_I2C_ADDRESS);
    if (this->wire_.write(command_data.data(), command_data.size()) != command_data.size()) {
...
bool PN532::ReadAck(uint32_t timeout) {
    ...
    if (this->wire_.readBytes(ack_buffer.data(), ack_buffer.size()) != ack_buffer.size()) {
        return false;
    }
    if (!std::equal(kPN532Ack, kPN532Ack + sizeof(kPN532Ack), ack_buffer.begin())) {
        return false;
    }
    return true;
}
...
bool PN532::ReadResponse(std::vector<uint8_t>& data, uint32_t timeout) {
    ...
    this->wire_.requestFrom(PN532_I2C_ADDRESS, static_cast<size_t>(1 + 3 + 2 + 3 + 1 + PN532_MAX_COMMAND_SIZE + 2), true);
    ...
    if (this->wire_.readBytes(preamble_data.data(), preamble_data.size()) != preamble_data.size()) {
        return false;
    }

    if (preamble_data[0] != PN532_PREAMBLE || preamble_data[1] != PN532_PREAMBLE || preamble_data[2] != PN532_STARTCODE2) {
        return false;
    }

    std::vector<uint8_t> length_data(2);
    if (this->wire_.readBytes(length_data.data(), length_data.size()) != length_data.size()) {
        return false;
    }
    ...
    if (length_data[0] == 0xFF && length_data[1] == 0xFF) {
        ...
        if (this->wire_.readBytes(extended_length_data.data(), extended_length_data.size()) != extended_length_data.size()) {
            return false;
        }

        if (extended_length_data[0] + extended_length_data[1] + extended_length_data[2] != 0) {
            return false;
        }

        length = (static_cast<uint16_t>(extended_length_data[0]) << 8) | extended_length_data[1];
        is_extended = true;
    } else {
    ...
    if (static_cast<uint8_t>(std::accumulate(response_data.begin(), response_data.end(), 0)) != 0) {
        return false;
    }
...
bool PN532::CommandExchange(const std::vector<uint8_t>& in_data, std::vector<uint8_t>& out_data, uint32_t timeout) {
    if (in_data.size() >= 255) {
        return false;
    }
...
bool PN532::GetFirmwareVersion(uint32_t& version) {
    std::vector<uint8_t> answer;
    if (!this->CommandExchange({ PN532_COMMAND_GETFIRMWAREVERSION_REQUEST }, answer)) {
        return false;
    }
    ...
    version = (answer[0] << 24ul) | (answer[1] << 16ul) | (answer[2] << 8ul) | answer[3];
    return true;
}

bool PN532::SetPassiveActivationRetries(uint8_t max_retries) {
    std::vector<uint8_t> answer;
    if (!this->CommandExchange({ PN532_COMMAND_RFCONFIGURATION_REQUEST, 5, 0xFF, 0x01, max_retries }, answer)) {
        return false;
    }
...
bool PN532::SAMConfigure() {
    std::vector<uint8_t> answer;
    if (!this->CommandExchange({ PN532_COMMAND_SAMCONFIGURATION_REQUEST, 1, 0x14, 1 }, answer)) {
        return false;
    }
