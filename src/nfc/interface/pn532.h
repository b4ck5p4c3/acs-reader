...
class PN532 : public NFCInterface {
    public:
        PN532(TwoWire wire, uint32_t scl_pin, uint32_t sda_pin, uint32_t irq_pin, uint32_t rst_pin) : 
            wire_(wire), scl_pin_(scl_pin), sda_pin_(sda_pin), irq_pin_(irq_pin), rst_pin_(rst_pin) {}

        virtual bool Init();
        virtual bool FindTag(NFCTagInfo& info, uint32_t timeout);
        virtual bool ApduExchange(const std::vector<uint8_t>& in_data, std::vector<uint8_t>& out_data, uint32_t timeout);
    ...
    private:
        bool CommandExchange(const std::vector<uint8_t>& in_data, std::vector<uint8_t>& out_data, uint32_t timeout = 1000);
